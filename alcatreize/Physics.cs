        ...
        public static void Register (AABB AABB)
        {
            switch (AABB)
            {
                case Pushbox pushbox:
                    ...
                    if (pushbox.IsStatic)
                    {
                        singleton.statisPushboxGrid.Add(pushbox, pushbox.GetBounds());
                    }
                    else
                    {
                        singleton.pushboxGrid.Add(singleton.registeredPushboxCount, pushbox, pushbox.GetBounds());
                    }
                    ...
                    break;
                case Hitbox hitbox:
                    singleton.hitboxGrid.Add(singleton.registeredHitboxCount, hitbox, hitbox.GetBounds());
                    hitbox.GridID = (int) singleton.registeredHitboxCount;
                    singleton.registeredHitboxCount++;
                    break;
                case Hurtbox hurtbox:
                    singleton.hurtboxGrid.Add(singleton.registeredHurtboxCount, hurtbox, hurtbox.GetBounds());
        ...
        public static void Update (AABB AABB)
        {
            switch (AABB)
            {
                case Pushbox pushbox:
                    ...
                    {
                        singleton.pushboxGrid.Update((uint)pushbox.GridID, pushbox.GetBounds());
                    }
                    break;
                case Hitbox hitbox:
                    if (hitbox.GridID == -1)
                    ...
                    {
                        singleton.hitboxGrid.Update((uint)hitbox.GridID, hitbox.GetBounds());
                    }
                    break;
                case Hurtbox hurtbox:
                    if (hurtbox.GridID == -1)
                    ...
                    {
                        singleton.hurtboxGrid.Update((uint)hurtbox.GridID, hurtbox.GetBounds());
                    }
                    break;
            }
        }
