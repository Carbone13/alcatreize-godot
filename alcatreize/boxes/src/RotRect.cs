...
namespace Dcrew.Spatial {
    ...
    public struct RotRect {
        ...
        public bool Intersects(Rectangle value) => Intersects(new RotRect(new Vector2(value.Location.X, value.Location.Y), new Vector2(value.Size.Width, value.Size.Height), 0, Vector2.Zero));
        ...
        public bool Contains(Point value) => Contains(new Vector2(value.X, value.Y));
        ...
        public bool Contains(Rectangle value) => Contains(new RotRect(new Vector2(value.Location.X, value.Location.Y), new Vector2(value.Size.Width, value.Size.Height), 0, Vector2.Zero));
        ...
        public bool Contains(RotRect value) {
            ...
            cos = (float)Math.Cos(value.Rotation);
            sin = (float)Math.Sin(value.Rotation);
            x = -value.Origin.X;
            y = -value.Origin.Y;
            w = value.Size.X + x;
            h = value.Size.Y + y;
        ...
        public Vector2 ClosestPoint(Vector2 xy) {
            ...
            p.X = Clamp(p.X, XY.X - Origin.X, XY.X + Size.X - Origin.X);
            p.Y =  Clamp(p.Y, XY.Y - Origin.Y, XY.Y + Size.Y - Origin.Y);
        ...
        bool IntersectsAnyEdge(RotRect value) {
            ...
            float rvCos = (float)Math.Cos(-value.Rotation),
                rvSin = (float)Math.Sin(-value.Rotation),
                vCos = (float)Math.Cos(value.Rotation),
                vSin = (float)Math.Sin(value.Rotation),
                vX = value.X,
                vY = value.Y,
            ...
            Vector2 ClosestPoint(Vector2 xy) {
                ...
                p.X = Clamp(p.X, vMinX, vMaxX);
                p.Y = Clamp(p.Y, vMinY, vMaxY);
                x = p.X - vX;
                y = p.Y - vY;
                xcos = x * vCos;
                ycos = y * vCos;
